...
class SQLiteGlobalBenchRepository:
    ...
    async def get(self, bench_id: str) -> GlobalBenchRecord | None:
        with self._database.transaction() as connection:
            row = connection.execute(
                "SELECT * FROM global_benches WHERE id = ?", (bench_id,)
            ).fetchone()
    ...
    async def save_snapshot(
        ...
        with self._database.transaction(immediate=True) as connection:
            ...
            if existing_row is not None:
                ...
                return _list_benches(connection, agent_id=snapshot.agent_id)
    ...
    async def get_snapshot(self, snapshot_id: UUID) -> StoredBenchSnapshot | None:
        with self._database.transaction() as connection:
            row = connection.execute(
                "SELECT * FROM bench_snapshots WHERE id = ?", (str(snapshot_id),)
            ).fetchone()
        return _snapshot_from_row(row) if row is not None else None

    async def mark_agent_offline(
        self, agent_id: UUID, observed_at: datetime
    ) -> builtins.list[GlobalBenchRecord]:
        ...
        with self._database.transaction(immediate=True) as connection:
            connection.execute(
                ...
                "WHERE agent_id = ? AND updated_at <= ? AND status != 'OFFLINE'",
                (timestamp, str(agent_id), timestamp),
            )
            return _list_benches(connection, agent_id=agent_id)
...
class SQLiteRemoteCommandRepository:
    ...
    async def get(self, command_id: UUID) -> RemoteCommand | None:
        with self._database.transaction() as connection:
            row = connection.execute(
                "SELECT * FROM remote_commands WHERE id = ?", (str(command_id),)
            ).fetchone()
    ...
    async def get_by_idempotency_key(
        ...
        with self._database.transaction() as connection:
            row = connection.execute(
                "SELECT * FROM remote_commands WHERE agent_id = ? AND idempotency_key = ?",
                (str(agent_id), idempotency_key),
            ).fetchone()
        return _command_from_row(row) if row is not None else None
...
class SQLiteDistributedOperationRepository:
    ...
    async def create(self, operation: DistributedOperation) -> DistributedOperation:
        with self._database.transaction(immediate=True) as connection:
            ...
            connection.execute(
                ...
                "(id, remote_command_id, agent_id, bench_id, reservation_id, operation_type, "
    ...
    async def get(self, operation_id: UUID) -> DistributedOperation | None:
        with self._database.transaction() as connection:
            row = connection.execute(
                "SELECT * FROM distributed_operations WHERE id = ?", (str(operation_id),)
            ).fetchone()
...
class SQLiteRemoteArtifactRepository:
    ...
    async def create(self, artifact: RemoteArtifactMetadata) -> RemoteArtifactMetadata:
        with self._database.transaction(immediate=True) as connection:
            row = connection.execute(
                ...
                (str(artifact.id), str(artifact.agent_id), str(artifact.local_artifact_id)),
            ).fetchone()
            if row is not None:
                existing = _artifact_from_row(row)
                _require_same_artifact_create(existing, artifact)
                return existing
            connection.execute(
                "INSERT INTO remote_artifacts "
                "(id, agent_id, local_artifact_id, command_id, operation_id, name, "
    ...
    async def get(self, artifact_id: UUID) -> RemoteArtifactMetadata | None:
        with self._database.transaction() as connection:
            row = connection.execute(
                "SELECT * FROM remote_artifacts WHERE id = ?", (str(artifact_id),)
            ).fetchone()
...
def _list_benches(connection: sqlite3.Connection, *, agent_id: UUID) -> list[GlobalBenchRecord]:
    rows = connection.execute(
        "SELECT * FROM global_benches WHERE agent_id = ? ORDER BY id", (str(agent_id),)
    ).fetchall()
