        ...
        *,
        ...
        with self._database.transaction() as connection:
            rows = connection.execute(
                ...
                "AND ("
                "(json_extract(command.payload_json, "
                "'$.reservation_lifecycle.management') IN ('workflow', 'caller') "
                "AND CAST(json_extract(command.payload_json, "
                "'$.reservation_lifecycle.release_after') AS TEXT) IN ('1', 'true')) "
                "OR (json_extract(command.payload_json, "
                "'$.reservation_lifecycle.management') IS NULL "
                "AND json_extract(reservation.record_json, "
                "'$.reservation.metadata.reservation_lifecycle') = 'workflow')"
                ") "
